    ...
    \begin{example}
        \begin{displaymath}
            \begin{split}
                ...
                \Gamma &\vdash \lambda s:Z \rightarrow Z. \lambda z: Z.
                    (s\, (s\, z)) : 
...
\begin{frame}{Typing relation is closed under substitution application}
    ...
    \end{theorem}
    \footnotesize
    \begin{proof}[Proof. By structural induction on the shape of term
    $t$]
        \begin{description} 
            \item[$t = x$] By theorem hp $\Gamma \vdash x:S_1$. By
            inversion lemma $x:S_1 \in \Gamma$. Get any substitution
            $\sigma$ and apply it memberwise, $x:\sigma S_1:\sigma
            \Gamma$ holds because if $\sigma S_1 = S_2$ for some
            $S_2$, then the same happens in $\sigma\Gamma$. By
            \emph{T-VAR} rule follow $\sigma\Gamma\vdash x:\sigma S_1$
            as required
            \item[$t = \lambda x:S_1.t_2$] By theorem hp $\Gamma
            \vdash \lambda x:S_1.t_2: S_2$. By inversion
            lemma $S_2 = S_1 \rightarrow S_3$ for some $S_3$ such that
            $\Gamma,x:S_1\vdash t_2:S_3$. By induction
            hp $\sigma\Gamma,x:\sigma S_1\vdash \sigma t_2:\sigma S_3$
            holds. By \emph{T-ABS} rule follow $\sigma\Gamma \vdash
            \lambda x:\sigma S_1. \sigma t_2:\sigma S_1\rightarrow
            \sigma S_3$ as required
            \item[$t = t_1 t_2$] By theorem hp $\Gamma \vdash t_1\,
            t_2:S_1$. By inversion lemma exists $S_2$ such that
            $\Gamma\vdash t_1:S_2\rightarrow S_1$ and $\Gamma\vdash
            t_2:S_2$. By induction hp $\sigma\Gamma\vdash\sigma
            t_1:\sigma S_2\rightarrow \sigma S_1$ and $\sigma\Gamma\vdash
            \sigma t_2:\sigma S_2$ hold. By \emph{T-APP} rule follow
            $\sigma\Gamma \vdash (\sigma t_1)\, (\sigma t_2):\sigma
            S_1$ as required
        \end{description} 
    \end{proof}
    \normalsize
...
\begin{frame}
    ...
    \begin{block}{Definition of \emph{solution} for $(\Gamma, t, S, \mathcal{C})$}
        Let $\sigma$ be a type substitution, $T_{1} \in T$ and suppose
            $\Gamma \vdash t : S \; \textbar_\mathcal{X} \; \mathcal{C}$ \\
...
\begin{frame}
    ...
    \end{displaymath}
        \onslide<6>
    \begin{displaymath}
             unify([ X \mapsto  Z \rightarrow C\rightarrow
                A]\{ Y \triangleq  Z \rightarrow
                    C\})\circ[ X \mapsto  Z \rightarrow C\rightarrow
    ...
    \end{displaymath}
        \onslide<8>
    \begin{displaymath}
             unify(\emptyset)\circ[ Y \mapsto  Z \rightarrow
                    C]\circ[ X \mapsto  Z \rightarrow C\rightarrow
                A]\circ[B\mapsto C\rightarrow A]\\
    \end{displaymath}
        \onslide<9>
    \begin{displaymath}
             []\circ[ Y \mapsto  Z \rightarrow
                    C]\circ[ X \mapsto  Z \rightarrow C\rightarrow
                A]\circ[B\mapsto C\rightarrow A]\\
    \end{displaymath}
    \end{overprint}
...
\begin{frame}
    ...
    \begin{displaymath}
         \sigma = []\circ[ Y \mapsto  Z \rightarrow
                C]\circ[ X \mapsto  Z \rightarrow C\rightarrow
            A]\circ[B\mapsto C\rightarrow A]
    \end{displaymath}

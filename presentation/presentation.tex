...
As we've seen in the last example, there exists possibly countable solutions
for a pair $(\Gamma, t)$. Are they related in some way? \\~\\
\pause
\begin{block}{Definition of constraint set}
    A \emph{constraint set} $\mathcal{C}$ is a set of equations 
    $\{S_i = T_i \}_{i \in \mathbb{N}}$ such that 
    $\mathcal{C} \subseteq \mathcal{A} \times T$
\end{block}

...
\begin{frame}{Introduction}
    ...
    abstraction have to say the \emph{concrete} type for the variable
    it introduce. \\~\\
    \pause
    ...
    \smiley) and say:
    \begin{center}
        I don't care to fix a \emph{concrete} type for my variable now,\\
    ...
    to \emph{all} variable we introduce in our program? When are our terms 
    well typed if no type is fixed at all? \\~\\
    \pause
...
\begin{frame}{Augment types with type variables}
    ...
    Let $X \in \mathcal{A}$:
    \begin{itemize}
        \item there's no typing rule that use the category $\mathcal{A}$
...
\begin{frame}{Substitutions}
    \begin{center}
        ``I know that type variable $X$ in my term $t$ stands for 
...
\begin{frame}{Typing relation is closed under substitution application}
    \begin{theorem}
        Let $\sigma$ be a type substitution. $\Gamma \vdash t:T 
        \rightarrow \sigma\Gamma \vdash \sigma t: \sigma T$
    \end{theorem}
    \begin{proof}
        TODO
    \end{proof}
...
\begin{frame}{Type inference}
    ...
    \begin{itemize}
        \item $\exists \sigma. \exists T_1: 
            \sigma \Gamma \vdash \sigma t : T_1$\\
            {\footnotesize It is always possible to find a type 
            ...
            whereever the user don't specify them
    \end{itemize}
\end{frame}

\begin{frame}
    Let $\Gamma$ be a context and $t$ be a term containing free variables.
    \begin{block}{Definition of solution}
        A \emph{solution} for $(\Gamma, t)$ is a pair $(\sigma, T_{1})$, where
        $\sigma$ is a type substitution and $T_{1} \in T$, such that 
        $\sigma \Gamma \vdash \sigma t:T_{1}$
    \end{block}
    \pause
    \begin{example}
